...
class Agent:
    ...
    def __setup_epsilon(self, **kwargs) -> None:
        self._epsilon = kwargs.get('epsilon', 0.0)
        self._desire_epsilon = kwargs.get('desire_epsilon', 0.01 if self.mode == 'train' else 0.0)
        self._games_to_reach_epsilon = kwargs.get('number_of_games_to_end_epsilon', 1000)
        self._epsilon_decreasing_factor = (self._desire_epsilon - self._epsilon) / self._games_to_reach_epsilon
    ...
    def get_action_object(self, action: int) -> torch.Tensor:
        move = [0, 0, 0]
        move[action] = 1
    ...
    def get_state(self, environment: Environment) -> torch.Tensor:
